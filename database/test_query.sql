...
SELECT p.product_ID, p.name, SUM(ol.quantity) as total_quantity, ol.unit
FROM Product p INNER JOIN Order_Line ol ON p.product_ID = ol.product_ID
GROUP BY p.product_ID, p.name, ol.unit;

-- Test 3: Print all Raw Materials (their ID, name, and default unit) and their total quantity used among Batch Consumptions
SELECT m.material_ID, m.name, SUM(bc.quantity_used) as total_quantity_used, m.default_unit
